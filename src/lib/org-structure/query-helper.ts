...
const log = useLogger({ module: 'org:query-helper' });
...
export class QueryHelper {
	...
	async getOrgStructureAt(
		...
		log.debug('Querying org structure at date', { date: date.toISOString() });
		...
		if (!version) {
			log.debug('No active version found for date');
			return null;
		}
	...
	async getEmployeeAssignmentAt(
		...
		log.debug('Querying employee assignment at date', { employeeId, date: date.toISOString() });
	...
	async getOrgUnitMembersAt(
		...
		log.debug('Querying org unit members at version', { orgUnitId, versionId });
	...
	> {
		...
		log.debug('Building timeline for employee in unit', { employeeId, orgUnitId });
	...
	async compareVersions(versionId1: string, versionId2: string): Promise<VersionComparison> {
		...
		log.debug('Comparing versions', { versionId1, versionId2 });
		...
		// Compare org units
		const units1 = new Map<string, any>(version1.structure.orgUnits.map((u: any) => [u._id, u]));
		const units2 = new Map<string, any>(version2.structure.orgUnits.map((u: any) => [u._id, u]));
		...
		// Compare positions
		const positions1 = new Map<string, any>(version1.structure.positions.map((p: any) => [p._id, p]));
		const positions2 = new Map<string, any>(version2.structure.positions.map((p: any) => [p._id, p]));
		...
		// Compare employees
		const employees1 = new Map<string, any>(
			version1.structure.employees.map((e: any) => [e.employeeId, e])
		);
		const employees2 = new Map<string, any>(
		...
		log.info('Comparison complete', { unitsAdded: comparison.orgUnits.added.length, employeesReassigned: comparison.employees.reassigned.length });
