...
const log = useLogger({ module: 'org:snapshot-builder' });
...
export async function buildOrgStructureSnapshot(
	...
	log.info('Building snapshot for organization', { organizationId });
	...
	log.info('Found snapshot data', { orgUnits: orgUnits.length, positions: positions.length, employees: identities.length });
	...
	for (const identity of identities) {
		try {
			...
			log.error('Error processing employee', { employeeId: identity.employeeId, error });
			// Continue with next employee
		}
	}

	log.info('Successfully created snapshot', { employeeCount: snapshotEmployees.length });
