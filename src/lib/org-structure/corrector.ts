...
export class VersionCorrector {
	...
	async correctEmployeeInSnapshot(
		...
		log.info('Correcting employee in version', { employeeId, versionId });
		...
		log.info('Employee corrected successfully', { employeeId });
	}
	...
	async syncOrgUnitChanges(
		...
		log.info('Syncing org unit changes', { orgUnitId });
		...
		log.info('Org unit synced successfully', { orgUnitId });
	}
	...
	async syncPositionChanges(
		...
		log.info('Syncing position changes', { positionId });
		...
		log.info('Position synced successfully', { positionId });
	}
	...
	async detectInconsistencies(versionId: string): Promise<InconsistencyIssue[]> {
		...
		log.info('Detecting inconsistencies in version', { versionId });
		...
		log.info('Found inconsistencies', { count: issues.length });
	...
	async autoFixInconsistencies(versionId: string): Promise<number> {
		...
		log.info('Attempting to auto-fix issues', { count: issues.length });
		...
		for (const issue of issues) {
			try {
				switch (issue.type) {
					...
					default:
						log.warn('Cannot auto-fix issue type', { type: issue.type });
				}
			} catch (error) {
				log.error('Error fixing issue', { issue, error });
			}
		}

		log.info('Auto-fixed issues', { count: fixed });
