...
const log = useLogger({ module: 'org:publisher' });
...
export class VersionPublisher {
	...
	async resumePublish(versionId: string): Promise<PublishResult> {
		...
		log.info('Resuming publish for version', { versionId });
	...
	private async archiveOldVersions(organizationId: string, currentVersionId: string) {
		...
		log.info('Archiving old versions', { organizationId });
		...
		log.info('Archived old versions', { count: result.modifiedCount });
	}
	...
	private async applyReassignments(versionId: string, reassignments: any[]) {
		...
		log.info('Applying reassignments', { count: reassignments.length });
		...
		for (const r of reassignments) {
			try {
				...
				if (!identity) {
					log.warn('Identity not found for employeeId', { employeeId: r.employeeId });
					continue;
				}
				...
				log.error('Error applying reassignment', { employeeId: r.employeeId, error });
				// Continue with next reassignment
			}
		}

		log.info('Updated identities', { count: updated });
	...
	private async createHistoryEntries(versionId: string, version: any) {
		...
		log.info('Creating history entries', { count: version.reassignments.length });
		...
		for (const r of version.reassignments) {
			try {
				...
				log.error('Error creating history entry', { employeeId: r.employeeId, error });
				// Continue with next entry
			}
		}

		log.info('Created history entries', { count: created });
