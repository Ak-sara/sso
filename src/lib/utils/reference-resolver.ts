...
const log = useLogger({ module: 'utils:reference-resolver' });
...
export async function buildCache(db: Db): Promise<ResolverCache> {
	log.info('Building reference cache');
	...
	log.info('Reference cache built', { orgs: cache.organizations.size, units: cache.orgUnits.size, positions: cache.positions.size, identities: cache.identities.size });
...
export function resolveOrganization(
	...
	if (isObjectIdFormat(value)) {
		log.warn('Skipping ObjectId string in CSV, treating as empty', { value });
