...
const log = useLogger({ module: 'utils:csv-exporter' });
			...
			if (resolveReferences && config.references && value instanceof ObjectId) {
				...
				if (refDef) {
					...
					if (resolvedValue) {
						...
						log.warn('Reference not found, outputting empty', { field: col.field, value: value.toString() });
						value = '';
					}
				}
			}
...
export async function exportCollectionToFile(
	...
	log.info('Exported collection to file', { collection: config.collection, outputPath });
}
