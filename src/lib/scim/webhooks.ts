...
async function deliverWebhook(
	...
	try {
		...
		log.info('Webhook delivered', { event: payload.event, url: subscription.webhookUrl });
	} catch (error: any) {
		log.warn('Webhook delivery failed, retrying', { attempt, error: error.message, url: subscription.webhookUrl });
		...
		if (attempt < subscription.retryPolicy.maxRetries) {
			...
			log.error('Webhook delivery failed permanently', { attempts: attempt, url: subscription.webhookUrl });
		}
	}
}
