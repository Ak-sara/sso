				...
				<div class="flex space-x-3">
					<form method="POST" action="?/fetchEntraUsers" use:enhance={() => {
						return async ({ result, update }) => {
							...
							if (data?.sampleUsers) {
								...
								availableEntraFields = extractAllFields(data.sampleUsers);
					...
					<form method="POST" action="?/fetchEntraUsers" use:enhance={() => {
						...
						return async ({ result, update }) => {
							const data = result.type === 'success' ? result.data as any : null;
							if (data?.csvContent) {
								const blob = new Blob([data.csvContent as string], { type: 'text/csv' });
								...
								alert(`✅ Exported ${data.userCount} users to CSV`);
							}
							isExporting = false;
							await update();
						};
