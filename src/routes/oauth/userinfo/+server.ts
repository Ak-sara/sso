...
export const GET: RequestHandler = async ({ locals }) => {
	const authorization = locals.vars.authorization;
	...
	try {
		const identity = await db.identities.findById(user.id);
		...
		if (identity) {
			...
			if (identity.identityType === 'employee') {
				...
				if (identity.organizationId) {
					...
					const organization = await db.organizations.findById(identity.organizationId);
					if (organization) {
				...
				if (identity.orgUnitId) {
					...
					const orgUnit = await db.orgUnits.findById(identity.orgUnitId);
					if (orgUnit) {
				...
				if (identity.positionId) {
					...
					const position = await db.positions.findById(identity.positionId);
					if (position) {
		...
		log.warn('Failed to fetch identity data for userinfo', { error: err });
