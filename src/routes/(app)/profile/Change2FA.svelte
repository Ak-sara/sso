...
<div class="grid p-2 gap-4">
	...
	{#if form?.backupCodes}
		<div class="bg-yellow-50 border-2 border-yellow-300 rounded-lg p-6">
			...
			<button class="w-full px-4 py-2 bg-yellow-600 text-white rounded-md hover:bg-yellow-700"
				onclick={() => {
			...
			<form method="POST" action="?/enable" use:enhance class="space-y-4">
				{#if form?.otpSent}
					<div>
						...
						<input class="w-full px-4 py-2 border rounded-md focus:ring-2 focus:ring-indigo-500"
							type="text"
							id="otpCode"
							name="otpCode"
							required
			...
			<form method="POST" action="?/disable" use:enhance class="space-y-4">
				{#if form?.disableOtpSent}
					<div>
						...
						<input class="w-full px-4 py-2 border rounded-md focus:ring-2 focus:ring-indigo-500"
							type="text" id="otpCode" name="otpCode"
							required maxlength="6" placeholder="000000" />
						<p class="text-xs text-gray-500 mt-1">Enter the code sent to your email</p>
					</div>
				{/if}

				<button class="w-full px-4 py-2 bg-red-600 text-white font-semibold rounded-md hover:bg-red-700"
					type="submit" >
