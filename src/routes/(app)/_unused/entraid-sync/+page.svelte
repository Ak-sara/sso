<script lang="ts">
	...
	const log = useLogger({ module: 'app:entraid-sync' });
	...
	// Field mapping state
	let fieldMapping: Record<string, { entraField: string; enabled: boolean; direction: string }> = $state(data.config?.fieldMapping as any || {
