...
const log = useLogger({ module: 'scim:bulk' });
...
export const POST: RequestHandler = async ({ locals, request, url }) => {
	...
	try {
		...
		// Check payload size
		const contentLength = locals.headers.get('content-length');
		...
		const bulkRequest: BulkRequest = locals.body as BulkRequest;
		...
		log.error('SCIM Bulk error', { error: err });
...
async function processUserOperation(
	...
	switch (operation.method) {
		case 'POST': {
			const newEmployee = await db.identities.insertOne({
				...
				isActive: !!operation.data.active, employmentStatus: operation.data.active ? 'active' : 'terminated',
			} as any);
		...
		case 'PUT': {
			...
			const didUpdate = await db.identities.updateById(resourceId, {
				firstName: operation.data.name?.givenName,
				lastName: operation.data.name?.familyName,
			...
			if (!didUpdate) throw new Error('User not found');
			const updatedEmployee = await db.identities.findById(resourceId);
			if (!updatedEmployee) throw new Error('User not found');
		...
		case 'PATCH': {
			...
			for (const op of operation.data.Operations || []) {
				if (op.op === 'replace') {
					if (op.path === 'active') {
						updates.isActive = !!op.value; updates.employmentStatus = op.value ? 'active' : 'terminated';
					}
				}
			}

			const didPatch = await db.identities.updateById(resourceId, updates);
			if (!didPatch) throw new Error('User not found');
			const patchedEmployee = await db.identities.findById(resourceId);
			if (!patchedEmployee) throw new Error('User not found');

			const scimUser = await employeeToScimUser(patchedEmployee, baseUrl);
		...
		case 'DELETE': {
			...
			await db.identities.updateById(resourceId, { employmentStatus: 'terminated', isActive: false } as any);
...
async function processGroupOperation(
	...
	switch (operation.method) {
		case 'POST': {
			const newOrgUnit = await db.orgUnits.insertOne({
		...
		case 'PATCH': {
			...
			const updates: any = { name: operation.data.displayName };
			const didUpdate = await db.orgUnits.updateById(resourceId, updates);
			if (!didUpdate) throw new Error('Group not found');
			const updatedOrgUnit = await db.orgUnits.findById(resourceId);
			if (!updatedOrgUnit) throw new Error('Group not found');
		...
		case 'DELETE': {
			...
			await db.orgUnits.updateById(resourceId, { isActive: false } as any);
