...
const log = useLogger({ module: 'scim:webhooks' });

const scimWebhooks = new Repository<WebhookSubscription>(lazy, 'scim_webhooks');
...
export const GET: RequestHandler = async ({ request, url }) => {
	try {
		...
		const subscriptions = await scimWebhooks.find({ clientId: auth.clientId });
		...
		log.error('Get webhooks error', { error: err });
		throw err;
	}
};
...
export const POST: RequestHandler = async ({ locals, request, url }) => {
	try {
		...
		log.error('Register webhook error', { error: err });
...
export const DELETE: RequestHandler = async ({ params, request, url }) => {
	try {
		...
		// Verify ownership
		const webhook = await scimWebhooks.findById(webhookId);
		...
		log.error('Delete webhook error', { error: err });
		if (err.status) throw err;
